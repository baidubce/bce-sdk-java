...
public abstract class AbstractMediaTest {  
    ...
    static String   NOTIFICATION_ENDPOINT = "http://bce.baidu.com";
    ...
    protected void checkNotificationExist(String expectedName, boolean expected) {
        ...
        if (expected) {
            assertTrue("Failed to find expected notification: "
                            + expectedName + ". \n " + resp,
                    names.contains(expectedName));
        } else {
            assertFalse("The pipeline: " + expectedName
                            + " still exists. \n " + resp,
                    names.contains(expectedName));
        }
    }
