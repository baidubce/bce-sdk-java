...
/**
 * The test cases for creating job
 */
public class CreateJobTest extends AbstractMediaTest {
    ...
    /**
     * test create job successfully with function  CreateTranscodingJob(CreateTranscodingJobRequest request)
     */
    @Test
    public void testCreateJobNormal1() {
        ...
        request.setSource(source);
        CreateTranscodingJobResponse jobResp = jobClient.createTranscodingJob(request);
        jobId = jobResp.getJobId();
        assertTrue("Invalid job ID: " + jobId, jobId != null
                && jobId.trim().length() > 0);
    }
    ...
    /**
     * test createJob(CreateJobRequest request) with source clips
     */
    @Test
    public void testCreateJobNormal2() {
        pipelineName = pipelineForJob;
        SourceClip clip = new SourceClip()
                .withBucket(AbstractMediaTest.SOURCE_BUCKET)
                .withSourceKey(AbstractMediaTest.SOURCE_KEY);
        presetName = presetNameSystem;
        CreateTranscodingJobResponse jobResp = jobClient.createTranscodingJob(pipelineName, Arrays.asList(clip),
                AbstractMediaTest.TARGET_KEY, presetName);
        jobId = jobResp.getJobId();
        assertTrue("Invalid job ID: " + jobId, jobId != null
                && jobId.trim().length() > 0);
    }
