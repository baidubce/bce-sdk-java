...
public class CreateThumbNailJobTest extends AbstractMediaTest {
    ...
    public void testMethodOfCreateThumbnailJobRequest() {  
        ...
        request.setSource(source); 
        ThumbnailTarget target = new ThumbnailTarget().withSizingPolicy("keep");
        ThumbnailCapture capture = new ThumbnailCapture().withMode("manual").withSkipBlackFrame(true)
    ...
    /**
     * test method of CreateThumbnailJobRequest with highlight mode
     */
    @Test
    public void testMethodOfCreateThumbnailJobRequestWithHighlight() {
        ...
        assertEquals(source.getKey(), sourceKey);
        request.setSource(source);
        ThumbnailTarget target = new ThumbnailTarget().withFormat("mp4");
        HighlightOutputCfg highlightOutputCfg = new HighlightOutputCfg();
        highlightOutputCfg.setDurationInSecond((float)1);
        ThumbnailCapture capture = new ThumbnailCapture().withMode("highlight")
    ...
    /**
     * test method of CreateThumbnailJob with spriteOutCfg
     */
    @Test
    public void testMethodOfCreateThumbnailJobWithSpriteOutCfg() {
        pipelineName = pipelineForJob;
        sourceKey = AbstractMediaTest.SOURCE_KEY4;
        ...
        ThumbnailCapture capture = new ThumbnailCapture().withMode("manual")
                .withSpriteOutputCfg(spriteOutputCfg);
        thumbNailJobId = mediaClient.createThumbnailJob(pipelineName,
                sourceKey, target, capture).getJobId();
        assertTrue("Invalid thumbnail job ID: " + thumbNailJobId, thumbNailJobId != null
                && thumbNailJobId.trim().length() > 0);
     ...
     */
    @Test
    public void testMethodOfCreateThumbnailJobWithSpriteOutCfgInTarget() {
        pipelineName = pipelineForJob;
        sourceKey = AbstractMediaTest.SOURCE_KEY4;
        SpriteOutputCfg spriteOutputCfg = new SpriteOutputCfg();
        spriteOutputCfg.setRows(5);
        ThumbnailTarget target = new ThumbnailTarget().withSpriteOutputCfg(spriteOutputCfg);
        ThumbnailCapture capture = new ThumbnailCapture().withMode("manual");
        thumbNailJobId = mediaClient.createThumbnailJob(pipelineName,
                sourceKey, target, capture).getJobId();
        assertTrue("Invalid thumbnail job ID: " + thumbNailJobId, thumbNailJobId != null
                && thumbNailJobId.trim().length() > 0);
        GetThumbnailJobResponse resp = mediaClient.getThumbnailJob(thumbNailJobId);
        assertNotNull(resp.getTarget().getSpriteOutputCfg());
    }
    ...
    public void testCreateThumbNailJobPipelineNull() {        
        ...
        bceEx.expectMessage("request pipelineName should not be null or empty string.");
        thumbNailJobId = mediaClient.createThumbnailJob(pipelineName,
                sourceKey, target, capture).getJobId();              
    } 
     ...
     * error specific in job error info
     */
    /*@Test
    ...
    public void testCreateThumbNailJobSourceKeyEmpty() {        
        ...
        ThumbnailCapture capture = new ThumbnailCapture(); 
        bceEx.expect(IllegalArgumentException.class);
        bceEx.expectMessage("request sourceKey should not be null or empty string.");
        thumbNailJobId = mediaClient.createThumbnailJob(pipelineName,
                sourceKey, target, capture).getJobId();
    }
    ...
    public void testCreateThumbNailJobSourceNull() {  
        ...
        request.setSource(source);  
        bceEx.expect(IllegalArgumentException.class);
        bceEx.expectMessage("request source should not be null or empty string.");
        thumbNailJobId = mediaClient.createThumbnailJob(request).getJobId();              
     ...
     * this test case has no means now.
     * intervalInSecond can zero, which means use video frame interval as interval.
     */ 
    /*@Test
    public void testCreateThumbNailJobIntervalInSecond0() {
        ...
        bceEx.expect(com.baidubce.BceServiceException.class);
        bceEx.expectMessage("intervalInSecond=must be greater than or equal to 0");
        thumbNailJobId = mediaClient.createThumbnailJob(pipelineName, sourceKey, target, capture).getJobId();
    }*/
