...
public class PipelineClientTest extends AbstractMediaTest {
    ...
    public void testUpdatePipeline() {
        pipelineName = convertName("ForTestUpdatePipeline");
        createPipeline();
        PipelineStatus pipeline = mediaClient.getPipeline(pipelineName).getPipeline();
        pipeline.getConfig().setCapacity(1);
        mediaClient.updatePipeline(new UpdatePipelineRequest().withPipeline(pipeline));
        assertEquals(mediaClient.getPipeline(pipelineName).getConfig().getCapacity().intValue(), 1);
    }
