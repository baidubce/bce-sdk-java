 ...
 */
public class UpdateStreamDestinationPushUrlTest extends AbstractLssTest {

    private String playDomain = "play.testsdk.iefay.xyz";

    private String app = "test_update_destination_push_url";
    ...
    public void test() {
        ...
        Assert.assertNull(response.getDestinationPushUrl());

        String destinationPushUrl = "rtmp://push.testsdk.iefay.xyz";
        lssClient.updateStreamDestinationPushUrl(playDomain, app, stream, destinationPushUrl);

        response = lssClient.getStream(playDomain, app, stream);
        Assert.assertEquals(destinationPushUrl, response.getDestinationPushUrl());
    }
