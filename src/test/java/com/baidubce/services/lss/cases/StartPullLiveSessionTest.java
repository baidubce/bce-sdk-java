 ...
 */
public class StartPullLiveSessionTest extends AbstractLssTest {
    ...
    public void tearDown() throws Exception {
        for (int i = 0; i < 60; i++) {
            ListSessionsResponse resp1 = lssClient.listSessions("CONNECTING");
            if (resp1.getSessions().size() != 0) {
        ...
        ListSessionsResponse resp2 = lssClient.listSessions("ONGOING");
        ...
        ListSessionsResponse resp3 = lssClient.listSessions();
        for (LiveSession session : resp3.getSessions()) {
            if (session.getDescription().equals(preSessionDescription)) {
    ...
    public void test() {
        ...
        System.out.println(response.toString());
        assertEquals(response.getStatus(), "CONNECTING");
    }
}
