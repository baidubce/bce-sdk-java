...
public class StsClientTest {
    ...
    public static class GetSigninSecurityTokenTest extends Base {
        @Test
        public void testOrdinary() {
            GetSigninSecurityTokenResponse response = this.client.getSigninSecurityToken(
                    new GetSigninSecurityTokenRequest("vcp_customer"));
            assertThat(response, hasProperty("userId"));
            assertThat(response, hasProperty("createTime"));
            assertThat(response, hasProperty("sessionToken"));
            assertThat(response, hasProperty("expiration", TestUtils.timeGapInSecondsGreaterThan(new Date(), 20)));
        }

        @Test
        public void testNonCloudAccount() {
            this.expectBceServiceException(404, "NotFound");
            this.client.getSigninSecurityToken(new GetSigninSecurityTokenRequest("non_vcp_customer"));
        }

        @Test
        public void testNonVcpAccount() {
            this.expectBceServiceException(403, "Forbidden");
            this.client.getSigninSecurityToken(new GetSigninSecurityTokenRequest("0fc2b2a826e54303955e92b903006ffd"));
        }
        ...
        public void testTemporaryCredential() {
            ...
            BceCredentials tempCredentials = new DefaultBceSessionCredentials(
                    ...
                    response.getSessionToken());
            this.expectBceServiceException(400, "InvalidRequest", "not support temporal credential");
            this.client.getSigninSecurityToken(new GetSigninSecurityTokenRequest("vcp_customer")
                    .withRequestCredentials(tempCredentials));
        }
    }
}
